...
abstract class Event_manager_metabox extends Event_manager_repeater_metabox {
    ...
    public function display_callback( $post ) {
		if ( "shop_order" == $post->type || "etn-stripe-order" == $post->type   ) {
			return;
		}
    ...
    public function save_meta_box_data( $post_id ) {
		...
		if ( "shop_order" == get_post_type($post_id) || "etn-stripe-order" == get_post_type($post_id) ) {
			return;
		}
    ...
    protected function update( $fields = null, $post = []) {
        ...
        foreach ( $fields as $field_key => $field ) {
            ...
            } elseif ( $field['type'] == 'date_range_picker'){
	...
	public function get_desc_markup( $item, $key ){
        ...
        <div class="<?php echo esc_html( $class ); ?>">
			<div class='etn-meta'>
				<?php echo Helper::kses(__( $text ),'eventin'); ?>
			</div>
		</div>
		<?php
    }
    ...
    public function get_button( $item, $key ){
		...
		<div class='<?php echo esc_attr( $class );?> etn_event_meta_field view-order-button'>
			...
			<div class='etn-meta'>
				<a href='<?php echo esc_url($item['url']) ?>' target='_blank' class='etn-btn-text <?php echo esc_attr( $btn_class );?>'>
    ...
    public function get_multi_checkbox( $item, $key ) {
        ...
        <div class="etn-label-item  <?php echo esc_attr( $class ); ?>">
            ...
            ?>
                <div class="etn-meta">
                    <?php
                        if ( ! empty( $item['inputs'] ) && count( $item['inputs'] ) > 0 ) {
                            foreach ( $item['inputs'] as $index => $item ) {
                                $checked   = in_array( $index, $value_arr ) ? 'checked' : '';
                                $item_key  = strtolower( str_replace( ' ', '_', $item ) );
                                ?>
                                    <input type="checkbox"
                                        name="<?php echo esc_attr( $key ) . '[' . $item_key . ']'; ?>"
                                        id="<?php esc_attr_e( $key . $index ); ?>"
                                        class="etn_multi_checkbox"
                                        value="<?php esc_attr_e( $item ); ?>"
                                        <?php esc_attr_e( $checked ); ?>/>
                                    <label class="etn_multi_label" for="<?php esc_attr_e( $key . $index ); ?>">
                                        <?php esc_html_e( $item ) ?>
                                    </label>
                                <?php
                            }
